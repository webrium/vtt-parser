...
private function extractTimestamps($vttTimestampString) {
    ...
    if (preg_match_all($timePattern, $vttTimestampString, $matches)) {
        ...
        if (!isset($matches[1][0]) || empty($matches[1][0])) {
            throw new \InvalidArgumentException("Start time not found in timestamp string($vttTimestampString)\nFile :".$this->full_path);
        }
        
        if (!isset($matches[1][1]) || empty($matches[1][1])) {
            throw new \InvalidArgumentException("End time not found in timestamp string($vttTimestampString)\nFile :".$this->full_path);
        }
